...
void loginUser() {
    ...
    for (seller& s : sellers) {
        if (s.getId() == id) {
            cout << "\nLogin successful. Welcome, " << s.getName() << " (Seller)!";
            sellerMenu(s);
            return;
        }
    }
    for (Buyer& b : buyers) {
        if (b.getId() == id) {
            cout << "\nLogin successful. Welcome, " << b.getName() << " (Buyer)!";
            buyerMenu(b);
            return;
        }
    }
...
void buyerMenu(Buyer& buyer) {
    ...
    do {
        ...
        if (cin.fail()) {
            cout << "Invalid input." << endl; cin.clear(); clearInputBuffer(); choice = 0;
        ...
        switch (choice) {
            ...
            case 7:
                accountDeleted = deleteAccount(buyer);
                if (accountDeleted) {
                    choice = 8;
                }
                break;
...
bool deleteAccount(Buyer& buyer) {
    cout << "\n--- Delete Account ---" << endl;
    cout << "WARNING: This action is irreversible and will delete all your data." << endl;
    char confirm;
    ...
    if (confirm == 'y' || confirm == 'Y') {
        ...
        auto seller_it = remove_if(sellers.begin(), sellers.end(), [userId](const seller& s) {
            return s.getId() == userId;
        });
        if (seller_it != sellers.end()) {
            sellers.erase(seller_it, sellers.end());
        }
        auto buyer_it = remove_if(buyers.begin(), buyers.end(), [userId](const Buyer& b) {
            return b.getId() == userId;
        });
        if (buyer_it != buyers.end()) {
            buyers.erase(buyer_it, buyers.end());
        }
        eshopBank.deleteAccountById(userId);
        cout << "Account has been successfully deleted. Returning to main menu." << endl;
        return true;
    } else {
        cout << "Account deletion cancelled. Returning to your menu." << endl;
        return false;
    }
}
...
void checkAccountStatus(Buyer& buyer) {
    ...
    for (const auto& s : sellers) {
        if (s.getId() == buyer.getId()) {
            cout << " & Seller" << endl;
            cout << "\n--- Store Details ---" << endl;
            cout << "Store Name: " << s.getStoreName() << endl;
            cout << "Store Address: " << s.getStoreAddress() << endl;
            cout << "Store Phone: " << s.getStorePhoneNumber() << endl;
            cout << "Store Email: " << s.getStoreEmail() << endl;
            isSeller = true;
            break;
        }
    }
    if (!isSeller) cout << endl;
    cout << "\n--- Banking Details ---" << endl;
    buyer.getAccount()->printInfo();
}

void upgradeToSeller(Buyer& buyer) {
    if (buyer.getAccount()->getBalance() <= 0) {
        cout << "Upgrade failed. You must have a positive balance." << endl;
        return;
    }
    for (const auto& s : sellers) {
        if (s.getId() == buyer.getId()) {
    ...
    cout << "Congratulations! You are now a seller." << endl;
    cout << "Please log out and log in again to access the seller menu." << endl;
}
...
void bankingFunctions(Buyer& buyer) {
    ...
    do {
        ...
        cout << "4. Return to Buyer Menu" << endl;
        cout << "Enter your choice: ";
        cin >> choice;
        ...
        switch (choice) {
            ...
            case 2: {
                ...
                cin >> amount;
                 if (!cin.fail() && amount > 0) {
                    buyer.getAccount()->withdrawBalance(amount);
                } else {
                    cout << "Invalid amount." << endl;
                }
                clearInputBuffer();
...
void browseStore(Buyer& buyer) {
    if (sellers.empty()) {
        cout << "\nThere are no stores available at the moment." << endl;
        return;
    }
    cout << "\n--- Available Stores ---" << endl;
    for (const auto& s : sellers) {
        cout << s.getSellerId() << ". " << s.getStoreName() << endl;
    }
    int storeId;
    cout << "Select a store to browse (enter ID): ";
    cin >> storeId;
    clearInputBuffer();
    seller* selectedSeller = nullptr;
    for (auto& s : sellers) {
        if (s.getSellerId() == storeId) {
            selectedSeller = &s;
            break;
        }
    }
    if (selectedSeller) {
        selectedSeller->viewItems();
        if (!selectedSeller->getItems().empty()) {
            int itemId;
            cout << "Enter the ID of the item to add to cart (or 0 to go back): ";
            cin >> itemId;
            clearInputBuffer();
            if (itemId != 0) {
                bool itemFound = false;
                for (const auto& item : selectedSeller->getItems()) {
                    if (item.getId() == itemId) {
                        buyer.getCart().addItem(item, selectedSeller->getSellerId());
                        itemFound = true;
                        break;
                    }
                }
                if (!itemFound) {
                    cout << "Item ID not found in this store." << endl;
                }
            }
        }
    } else {
        ...
        switch (choice) {
            case 1: {
                ...
                if (!buyer.getCart().getItems().empty()) {
                    sellerId = buyer.getCart().getItems()[0].second;
                }
                for(const auto& pair : buyer.getCart().getItems()){
                    itemsInCart.push_back(pair.first);
                }
                int newOrderId = orders.size() + 1;
                Order newOrder(newOrderId, buyer.getId(), sellerId, itemsInCart);
                orders.push_back(newOrder);
                buyer.getCart().clearCart();
                cout << "Checkout successful! An invoice has been generated." << endl;
                cout << "Your Order ID is " << newOrderId << ". Please proceed to Payment menu." << endl;
                return;
            }
            case 2: {
                if (buyer.getCart().isEmpty()) {
                ...
                cin >> itemIdToRemove;
                clearInputBuffer();
                buyer.getCart().removeItem(itemIdToRemove);
                break;
            }
            case 3:
                break;
...
void paymentMenu(Buyer& buyer) {
    ...
    for (const auto& order : orders) {
        if (order.getBuyerId() == buyer.getId() && order.getStatus() == UNPAID) {
    ...
    if (!hasInvoice) {
        cout << "You have no unpaid invoices." << endl;
        return;
    }
    int orderId;
    ...
    for (auto& o : orders) {
        if (o.getOrderId() == orderId && o.getBuyerId() == buyer.getId()) {
            orderToPay = &o;
            break;
        }
    }
    if (orderToPay && orderToPay->getStatus() == UNPAID) {
        ...
        if (balance >= total) {
            ...
            if (confirm == 'y' || confirm == 'Y') {
                ...
                for (auto& s : sellers) {
                    if (s.getSellerId() == orderToPay->getSellerId()) {
                        s.getAccount()->addBalance(total);
                        break;
                    }
                }
                orderToPay->setStatus(PAID);
                cout << "Payment successful! Your order status is now PAID." << endl;
            } else {
        ...
        cout << "Invalid Order ID or order is already paid." << endl;
    }
}
...
void sellerMenu(seller& currentSeller) {
    ...
    do {
        ...
        switch (choice) {
            case 1: addInventory(currentSeller); break;
            case 2: removeInventory(currentSeller); break;
            case 3: currentSeller.viewItems(); break;
            case 4: viewSellerOrders(currentSeller); break;
            case 5: {
                char confirm;
                cout << "Are you sure you want to logout? (y/n): ";
                cin >> confirm;
...
void addInventory(seller& currentSeller) {
    ...
    clearInputBuffer();
    currentSeller.addNewItem(id, name, quantity, price);
    cout << "Item '" << name << "' added to inventory." << endl;
}

void removeInventory(seller& currentSeller) {
    currentSeller.viewItems();
    if (currentSeller.getItems().empty()) return;
    int itemId;
    cout << "Enter the ID of the item to remove: ";
    cin >> itemId;
    clearInputBuffer();
    currentSeller.removeItem(itemId);
}
...
void viewSellerOrders(seller& currentSeller) {
    ...
    for (const auto& order : orders) {
        if (order.getSellerId() == currentSeller.getSellerId() && order.getStatus() == PAID) {
            cout << "Order ID: " << order.getOrderId()
    ...
    if (!hasOrders) {
        cout << "You have no paid orders." << endl;
    }
}